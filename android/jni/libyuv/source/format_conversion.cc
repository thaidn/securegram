...
static uint32 GenerateSelector(int select0, int select1) {
  return (uint32)(select0) | (uint32)((select1 + 4) << 8) |
         (uint32)((select0 + 8) << 16) | (uint32)((select1 + 12) << 24);
}

static int MakeSelectors(const int blue_index, const int green_index,
                         const int red_index, uint32 dst_fourcc_bayer,
                         uint32* index_map) {
...
LIBYUV_API
int ARGBToBayer(const uint8* src_argb, int src_stride_argb, uint8* dst_bayer,
                int dst_stride_bayer, int width, int height,
...
#endif
  if (MakeSelectors(blue_index, green_index, red_index, dst_fourcc_bayer,
                    index_map)) {
    return -1;  // Bad FourCC
  }
...
LIBYUV_API
int BayerToARGB(const uint8* src_bayer, int src_stride_bayer, uint8* dst_argb,
...
// Converts any Bayer RGB format to ARGB.
LIBYUV_API
int BayerToI420(const uint8* src_bayer, int src_stride_bayer, uint8* dst_y,
...

  {
    ...
    for (y = 0; y < height - 1; y += 2) {
      ...
      BayerRow1(src_bayer + src_stride_bayer, -src_stride_bayer, row + kRowSize,
                width);
...
LIBYUV_API
int I420ToBayer(const uint8* src_y, int src_stride_y, const uint8* src_u,
                int src_stride_u, const uint8* src_v, int src_stride_v,
                uint8* dst_bayer, int dst_stride_bayer, int width, int height,
                uint32 dst_fourcc_bayer) {
...
#define MAKEBAYERFOURCC(BAYER)                                                 \
  ...
  int Bayer##BAYER##ToI420(const uint8* src_bayer, int src_stride_bayer,       \
                           ...
                           int dst_stride_u, uint8* dst_v, int dst_stride_v,   \
                           int width, int height) {                            \
    return BayerToI420(src_bayer, src_stride_bayer, dst_y, dst_stride_y,       \
                       dst_u, dst_stride_u, dst_v, dst_stride_v, width,        \
                       height, FOURCC_##BAYER);                                \
  }                                                                            \
                                                                               \
  LIBYUV_API                                                                   \
  int I420ToBayer##BAYER(                                                      \
      const uint8* src_y, int src_stride_y, const uint8* src_u,                \
      int src_stride_u, const uint8* src_v, int src_stride_v,                  \
      uint8* dst_bayer, int dst_stride_bayer, int width, int height) {         \
    return I420ToBayer(src_y, src_stride_y, src_u, src_stride_u, src_v,        \
                       src_stride_v, dst_bayer, dst_stride_bayer, width,       \
                       height, FOURCC_##BAYER);                                \
  }                                                                            \
                                                                               \
  LIBYUV_API                                                                   \
  int ARGBToBayer##BAYER(const uint8* src_argb, int src_stride_argb,           \
                         uint8* dst_bayer, int dst_stride_bayer, int width,    \
                         int height) {                                         \
    return ARGBToBayer(src_argb, src_stride_argb, dst_bayer, dst_stride_bayer, \
                       width, height, FOURCC_##BAYER);                         \
  }                                                                            \
                                                                               \
  LIBYUV_API                                                                   \
  int Bayer##BAYER##ToARGB(const uint8* src_bayer, int src_stride_bayer,       \
                           ...
                           int height) {                                       \
    return BayerToARGB(src_bayer, src_stride_bayer, dst_argb, dst_stride_argb, \
                       width, height, FOURCC_##BAYER);                         \
  }
