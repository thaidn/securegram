...
#define YANY(NAMEANY, I420TORGB_SIMD, I420TORGB_C, UV_SHIFT, BPP, MASK)      \
  void NAMEANY(const uint8* y_buf, const uint8* u_buf, const uint8* v_buf,   \
    ...
    I420TORGB_C(y_buf + n, u_buf + (n >> UV_SHIFT), v_buf + (n >> UV_SHIFT), \
...
YANY(I422ToARGB1555Row_Any_SSSE3, I422ToARGB1555Row_SSSE3, I422ToARGB1555Row_C,
     1, 2, 7)
YANY(I422ToRGB565Row_Any_SSSE3, I422ToRGB565Row_SSSE3, I422ToRGB565Row_C, 1, 2,
     7)
...
YANY(I422ToARGB4444Row_Any_NEON, I422ToARGB4444Row_NEON, I422ToARGB4444Row_C, 1,
     2, 7)
YANY(I422ToARGB1555Row_Any_NEON, I422ToARGB1555Row_NEON, I422ToARGB1555Row_C, 1,
     2, 7)
...
#define NV2NY(NAMEANY, NV12TORGB_SIMD, NV12TORGB_C, UV_SHIFT, BPP)      \
  void NAMEANY(const uint8* y_buf, const uint8* uv_buf, uint8* rgb_buf, \
               int width) {                                             \
    int n = width & ~7;                                                 \
    NV12TORGB_SIMD(y_buf, uv_buf, rgb_buf, n);                          \
    NV12TORGB_C(y_buf + n, uv_buf + (n >> UV_SHIFT), rgb_buf + n * BPP, \
                width & 7);                                             \
  }
...
NV2NY(NV12ToRGB565Row_Any_SSSE3, NV12ToRGB565Row_SSSE3, NV12ToRGB565Row_C, 0, 2)
NV2NY(NV21ToRGB565Row_Any_SSSE3, NV21ToRGB565Row_SSSE3, NV21ToRGB565Row_C, 0, 2)
...
RGBANY(ARGBToRGB24Row_Any_SSSE3, ARGBToRGB24Row_SSSE3, ARGBToRGB24Row_C, 15, 4,
       3)
RGBANY(ARGBToRAWRow_Any_SSSE3, ARGBToRAWRow_SSSE3, ARGBToRAWRow_C, 15, 4, 3)
RGBANY(ARGBToRGB565Row_Any_SSE2, ARGBToRGB565Row_SSE2, ARGBToRGB565Row_C, 3, 4,
       2)
...
RGBANY(I400ToARGBRow_Any_SSE2, I400ToARGBRow_Unaligned_SSE2, I400ToARGBRow_C, 7,
       1, 4)
#endif
...
RGBANY(RGB24ToARGBRow_Any_SSSE3, RGB24ToARGBRow_SSSE3, RGB24ToARGBRow_C, 15, 3,
       4)
RGBANY(RAWToARGBRow_Any_SSSE3, RAWToARGBRow_SSSE3, RAWToARGBRow_C, 15, 3, 4)
RGBANY(RGB565ToARGBRow_Any_SSE2, RGB565ToARGBRow_SSE2, RGB565ToARGBRow_C, 7, 2,
       4)
...
RGBANY(ARGBToRGB565Row_Any_NEON, ARGBToRGB565Row_NEON, ARGBToRGB565Row_C, 7, 4,
       2)
...
BAYERANY(ARGBToBayerRow_Any_SSSE3, ARGBToBayerRow_SSSE3, ARGBToBayerRow_C, 7, 4,
         1)
#endif
#if defined(HAS_ARGBTOBAYERROW_NEON)
BAYERANY(ARGBToBayerRow_Any_NEON, ARGBToBayerRow_NEON, ARGBToBayerRow_C, 7, 4,
         1)
#endif
...
#define YANY(NAMEANY, ARGBTOY_SIMD, SBPP, BPP, NUM)                            \
  void NAMEANY(const uint8* src_argb, uint8* dst_y, int width) {               \
    ...
    ARGBTOY_SIMD(src_argb + (width - NUM) * SBPP, dst_y + (width - NUM) * BPP, \
                 NUM);                                                         \
  }
...
#define YANY(NAMEANY, ARGBTOY_SIMD, ARGBTOY_C, SBPP, BPP, MASK)    \
  void NAMEANY(const uint8* src_argb, uint8* dst_y, int width) {   \
    ...
    ARGBTOY_C(src_argb + n * SBPP, dst_y + n * BPP, width & MASK); \
  }

// Attenuate is destructive so last16 method can not be used due to overlap.
#ifdef HAS_ARGBATTENUATEROW_SSSE3
YANY(ARGBAttenuateRow_Any_SSSE3, ARGBAttenuateRow_SSSE3, ARGBAttenuateRow_C, 4,
     4, 3)
#endif
#ifdef HAS_ARGBATTENUATEROW_SSE2
YANY(ARGBAttenuateRow_Any_SSE2, ARGBAttenuateRow_SSE2, ARGBAttenuateRow_C, 4, 4,
     3)
#endif
...
YANY(ARGBAttenuateRow_Any_AVX2, ARGBAttenuateRow_AVX2, ARGBAttenuateRow_C, 4, 4,
     7)
#endif
...
YANY(ARGBAttenuateRow_Any_NEON, ARGBAttenuateRow_NEON, ARGBAttenuateRow_C, 4, 4,
     7)
#endif
#undef YANY
...
#define UVANY(NAMEANY, ANYTOUV_SIMD, ANYTOUV_C, BPP, MASK)               \
  void NAMEANY(const uint8* src_argb, int src_stride_argb, uint8* dst_u, \
               uint8* dst_v, int width) {                                \
    int n = width & ~MASK;                                               \
    ANYTOUV_SIMD(src_argb, src_stride_argb, dst_u, dst_v, n);            \
    ANYTOUV_C(src_argb + n * BPP, src_stride_argb, dst_u + (n >> 1),     \
              dst_v + (n >> 1), width & MASK);                           \
  }
...
UVANY(ARGBToUVJRow_Any_SSSE3, ARGBToUVJRow_Unaligned_SSSE3, ARGBToUVJRow_C, 4,
      15)
...
#define UV422ANY(NAMEANY, ANYTOUV_SIMD, ANYTOUV_C, BPP, MASK, SHIFT)         \
  void NAMEANY(const uint8* src_uv, uint8* dst_u, uint8* dst_v, int width) { \
    ...
    ANYTOUV_C(src_uv + n * BPP, dst_u + (n >> SHIFT), dst_v + (n >> SHIFT),  \
              width & MASK);                                                 \
  }
...
UV422ANY(YUY2ToUV422Row_Any_AVX2, YUY2ToUV422Row_AVX2, YUY2ToUV422Row_C, 2, 31,
         1)
UV422ANY(UYVYToUV422Row_Any_AVX2, UYVYToUV422Row_AVX2, UYVYToUV422Row_C, 2, 31,
         1)
#endif
#ifdef HAS_ARGBTOUVROW_SSSE3
...
UV422ANY(ARGBToUV444Row_Any_NEON, ARGBToUV444Row_NEON, ARGBToUV444Row_C, 4, 7,
         0)
UV422ANY(ARGBToUV422Row_Any_NEON, ARGBToUV422Row_NEON, ARGBToUV422Row_C, 4, 15,
         1)
UV422ANY(ARGBToUV411Row_Any_NEON, ARGBToUV411Row_NEON, ARGBToUV411Row_C, 4, 31,
         2)
UV422ANY(YUY2ToUV422Row_Any_NEON, YUY2ToUV422Row_NEON, YUY2ToUV422Row_C, 2, 15,
         1)
UV422ANY(UYVYToUV422Row_Any_NEON, UYVYToUV422Row_NEON, UYVYToUV422Row_C, 2, 15,
         1)
#endif
#undef UV422ANY
...
#define SPLITUVROWANY(NAMEANY, ANYTOUV_SIMD, ANYTOUV_C, MASK)                \
  void NAMEANY(const uint8* src_uv, uint8* dst_u, uint8* dst_v, int width) { \
    int n = width & ~MASK;                                                   \
    ANYTOUV_SIMD(src_uv, dst_u, dst_v, n);                                   \
    ANYTOUV_C(src_uv + n * 2, dst_u + n, dst_v + n, width & MASK);           \
  }
...
#define MERGEUVROW_ANY(NAMEANY, ANYTOUV_SIMD, ANYTOUV_C, MASK)        \
  void NAMEANY(const uint8* src_u, const uint8* src_v, uint8* dst_uv, \
               int width) {                                           \
    int n = width & ~MASK;                                            \
    ANYTOUV_SIMD(src_u, src_v, dst_uv, n);                            \
    ANYTOUV_C(src_u + n, src_v + n, dst_uv + n * 2, width & MASK);    \
  }
...
#define MATHROW_ANY(NAMEANY, ARGBMATH_SIMD, ARGBMATH_C, MASK)          \
  void NAMEANY(const uint8* src_argb0, const uint8* src_argb1,         \
    ...
    ARGBMATH_C(src_argb0 + n * 4, src_argb1 + n * 4, dst_argb + n * 4, \
               width & MASK);                                          \
  }
...
#define YANY(NAMEANY, ARGBTOY_SIMD, ARGBTOY_C, SBPP, BPP, MASK)               \
  void NAMEANY(const uint8* src_argb, uint8* dst_argb, const uint8* shuffler, \
               int width) {                                                   \
    int n = width & ~MASK;                                                    \
    ARGBTOY_SIMD(src_argb, dst_argb, shuffler, n);                            \
    ARGBTOY_C(src_argb + n * SBPP, dst_argb + n * BPP, shuffler,              \
              width & MASK);                                                  \
  }

#ifdef HAS_ARGBSHUFFLEROW_SSE2
YANY(ARGBShuffleRow_Any_SSE2, ARGBShuffleRow_SSE2, ARGBShuffleRow_C, 4, 4, 3)
#endif
#ifdef HAS_ARGBSHUFFLEROW_SSSE3
YANY(ARGBShuffleRow_Any_SSSE3, ARGBShuffleRow_Unaligned_SSSE3, ARGBShuffleRow_C,
     4, 4, 7)
#endif
#ifdef HAS_ARGBSHUFFLEROW_AVX2
YANY(ARGBShuffleRow_Any_AVX2, ARGBShuffleRow_AVX2, ARGBShuffleRow_C, 4, 4, 15)
#endif
#ifdef HAS_ARGBSHUFFLEROW_NEON
YANY(ARGBShuffleRow_Any_NEON, ARGBShuffleRow_NEON, ARGBShuffleRow_C, 4, 4, 3)
#endif
#undef YANY
...
#define NANY(NAMEANY, TERP_SIMD, TERP_C, SBPP, BPP, MASK)                      \
  void NAMEANY(uint8* dst_ptr, const uint8* src_ptr, ptrdiff_t src_stride_ptr, \
    ...
    TERP_C(dst_ptr + n * BPP, src_ptr + n * SBPP, src_stride_ptr,              \
           width & MASK, source_y_fraction);                                   \
  }

#ifdef HAS_INTERPOLATEROW_AVX2
NANY(InterpolateRow_Any_AVX2, InterpolateRow_AVX2, InterpolateRow_C, 1, 1, 32)
#endif
#ifdef HAS_INTERPOLATEROW_SSSE3
NANY(InterpolateRow_Any_SSSE3, InterpolateRow_Unaligned_SSSE3, InterpolateRow_C,
     1, 1, 15)
#endif
#ifdef HAS_INTERPOLATEROW_SSE2
NANY(InterpolateRow_Any_SSE2, InterpolateRow_Unaligned_SSE2, InterpolateRow_C,
     1, 1, 15)
#endif
