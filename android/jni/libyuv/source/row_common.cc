...
static __inline void YuvPixel(uint8 y, uint8 u, uint8 v, uint8* b, uint8* g,
                              uint8* r) {
...
void I422ToRGB24Row_C(const uint8* src_y, const uint8* src_u,
                      const uint8* src_v, uint8* rgb_buf, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
    YuvPixel(src_y[0], src_u[0], src_v[0], rgb_buf + 0, rgb_buf + 1,
             rgb_buf + 2);
...
void I411ToARGBRow_C(const uint8* src_y, const uint8* src_u, const uint8* src_v,
  ...
  for (x = 0; x < width - 3; x += 4) {
    ...
    YuvPixel(src_y[1], src_u[0], src_v[0], rgb_buf + 4, rgb_buf + 5,
             rgb_buf + 6);
    rgb_buf[7] = 255;
    YuvPixel(src_y[2], src_u[0], src_v[0], rgb_buf + 8, rgb_buf + 9,
             rgb_buf + 10);
    rgb_buf[11] = 255;
    YuvPixel(src_y[3], src_u[0], src_v[0], rgb_buf + 12, rgb_buf + 13,
             rgb_buf + 14);
    rgb_buf[15] = 255;
    src_y += 4;
    src_u += 1;
...
void NV12ToARGBRow_C(const uint8* src_y, const uint8* usrc_v, uint8* rgb_buf,
                     int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
    YuvPixel(src_y[0], usrc_v[0], usrc_v[1], rgb_buf + 0, rgb_buf + 1,
             rgb_buf + 2);
    rgb_buf[3] = 255;
    YuvPixel(src_y[1], usrc_v[0], usrc_v[1], rgb_buf + 4, rgb_buf + 5,
             rgb_buf + 6);
    rgb_buf[7] = 255;
    src_y += 2;
    usrc_v += 2;
...
void NV21ToARGBRow_C(const uint8* src_y, const uint8* src_vu, uint8* rgb_buf,
                     int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
...
void NV12ToRGB565Row_C(const uint8* src_y, const uint8* usrc_v,
...
void NV21ToRGB565Row_C(const uint8* src_y, const uint8* vsrc_u,
                       uint8* dst_rgb565, int width) {
  uint8 b0;
  uint8 g0;
  uint8 r0;
...
void YUY2ToARGBRow_C(const uint8* src_yuy2, uint8* rgb_buf, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
...
void UYVYToARGBRow_C(const uint8* src_uyvy, uint8* rgb_buf, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
...
void YToARGBRow_C(const uint8* src_y, uint8* rgb_buf, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
    YuvPixel(src_y[0], 128, 128, rgb_buf + 0, rgb_buf + 1, rgb_buf + 2);
    rgb_buf[3] = 255;
    YuvPixel(src_y[1], 128, 128, rgb_buf + 4, rgb_buf + 5, rgb_buf + 6);
    rgb_buf[7] = 255;
    src_y += 2;
    rgb_buf += 8;  // Advance 2 pixels.
  }
...
void YUY2ToUVRow_C(const uint8* src_yuy2, int src_stride_yuy2, uint8* dst_u,
...
void UYVYToUVRow_C(const uint8* src_uyvy, int src_stride_uyvy, uint8* dst_u,
                   uint8* dst_v, int width) {
  // Output a row of UV values.
  int x;
  for (x = 0; x < width; x += 2) {
...
void CumulativeSumToAverageRow_C(const int32* tl, const int32* bl, int w,
                                 int area, uint8* dst, int count) {
  float ooa = 1.0f / area;
  int i;
  for (i = 0; i < count; ++i) {
...
void InterpolateRow_C(uint8* dst_ptr, const uint8* src_ptr,
                      ptrdiff_t src_stride, int width, int source_y_fraction) {
