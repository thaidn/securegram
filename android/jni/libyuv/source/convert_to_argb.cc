...
LIBYUV_API
int ConvertToARGB(const uint8* sample, size_t sample_size, uint8* crop_argb,
                  int argb_stride, int crop_x, int crop_y, int src_width,
  ...
  switch (format) {
    ...
    case FOURCC_BGGR:
      ...
      r = BayerBGGRToARGB(src, src_width, crop_argb, argb_stride, crop_width,
                          inv_crop_height);
      break;

    case FOURCC_GBRG:
      src = sample + (src_width * crop_y + crop_x);
      r = BayerGBRGToARGB(src, src_width, crop_argb, argb_stride, crop_width,
                          inv_crop_height);
      break;

    case FOURCC_GRBG:
      src = sample + (src_width * crop_y + crop_x);
      r = BayerGRBGToARGB(src, src_width, crop_argb, argb_stride, crop_width,
                          inv_crop_height);
      break;

    case FOURCC_RGGB:
      src = sample + (src_width * crop_y + crop_x);
      r = BayerRGGBToARGB(src, src_width, crop_argb, argb_stride, crop_width,
                          inv_crop_height);
      break;
    ...
    case FOURCC_M420:
      ...
      break;
    //    case FOURCC_Q420:
    //      src = sample + (src_width + aligned_src_width * 2) * crop_y +
    //      crop_x;
    ...
    case FOURCC_I411: {
      ...
      const uint8* src_v = sample + src_width * abs_src_height +
                           ...
                           crop_x / 4;
      r = I411ToARGB(src_y, src_width, src_u, quarterwidth, src_v, quarterwidth,
                     crop_argb, argb_stride, crop_width, inv_crop_height);
      break;
    }
#ifdef HAVE_JPEG
  ...
  if (need_buf) {
    if (!r) {
      r = ARGBRotate(crop_argb, argb_stride, tmp_argb, tmp_argb_stride,
