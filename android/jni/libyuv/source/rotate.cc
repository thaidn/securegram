...
// void MirrorRow_NEON(const uint8* src, uint8* dst, int width);
// #define HAS_MIRRORROW_UV_NEON
// void MirrorUVRow_NEON(const uint8* src, uint8* dst_a, uint8* dst_b, int
// width);
// #define HAS_TRANSPOSE_WX8_NEON
...
void TransposeWx8_FAST_MIPS_DSPR2(const uint8* src, int src_stride, uint8* dst,
                                  int dst_stride, int width);
...
__declspec(naked) __declspec(align(16)) static void TransposeWx8_SSSE3(
    const uint8* src, int src_stride, uint8* dst, int dst_stride, int width) {
  __asm {
    push      edi
    push      esi
    push      ebp
...
__declspec(naked) __declspec(align(16)) static void TransposeUVWx8_SSE2(
    const uint8* src, int src_stride, uint8* dst_a, int dst_stride_a,
    uint8* dst_b, int dst_stride_b, int w) {
  __asm {
    push      ebx
    push      esi
    push      edi
...
#define HAS_TRANSPOSE_WX8_SSSE3
static void TransposeWx8_SSSE3(const uint8* src, int src_stride, uint8* dst,
                               int dst_stride, int width) {
  asm volatile(
      // Read in the data from the source pointer.
      // First round of bit swap.
...
#define HAS_TRANSPOSE_UVWX8_SSE2
static void TransposeUVWx8_SSE2(const uint8* src, int src_stride, uint8* dst_a,
                                int dst_stride_a, uint8* dst_b,
                                int dst_stride_b, int w) {
  asm volatile(
      // Read in the data from the source pointer.
      // First round of bit swap.
      ".p2align  2                                 \n"
...
static void TransposeWx8_C(const uint8* src, int src_stride, uint8* dst,
                           int dst_stride, int width) {
  int i;
  for (i = 0; i < width; ++i) {
...
static void TransposeWxH_C(const uint8* src, int src_stride, uint8* dst,
...
void RotatePlane180(const uint8* src, int src_stride, uint8* dst,
  ...
  if (TestCpuFlag(kCpuHasMIPS_DSPR2) && IS_ALIGNED(src, 4) &&
...
static void TransposeUVWx8_C(const uint8* src, int src_stride, uint8* dst_a,
                             int dst_stride_a, uint8* dst_b, int dst_stride_b,
                             int width) {
  int i;
  for (i = 0; i < width; ++i) {
...
static void TransposeUVWxH_C(const uint8* src, int src_stride, uint8* dst_a,
                             int dst_stride_a, uint8* dst_b, int dst_stride_b,
                             int width, int height) {
  int i;
...
void TransposeUV(const uint8* src, int src_stride, uint8* dst_a,
  ...
  if (TestCpuFlag(kCpuHasSSE2) && IS_ALIGNED(width, 8) && IS_ALIGNED(src, 16) &&
