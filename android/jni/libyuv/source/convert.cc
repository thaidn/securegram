...
static int I4xxToI420(const uint8* src_y, int src_stride_y, const uint8* src_u,
                      ...
                      int src_y_width, int src_y_height, int src_uv_width,
                      int src_uv_height) {
  const int dst_y_width = Abs(src_y_width);
  const int dst_y_height = Abs(src_y_height);
  const int dst_uv_width = SUBSAMPLE(dst_y_width, 1, 1);
  const int dst_uv_height = SUBSAMPLE(dst_y_height, 1, 1);
  if (src_y_width == 0 || src_y_height == 0 || src_uv_width == 0 ||
      src_uv_height == 0) {
    return -1;
  }
...
static void CopyPlane2(const uint8* src, int src_stride_0, int src_stride_1,
  ...
  if (TestCpuFlag(kCpuHasSSE2) && IS_ALIGNED(width, 32) &&
      IS_ALIGNED(src, 16) && IS_ALIGNED(src_stride_0, 16) &&
      IS_ALIGNED(src_stride_1, 16) && IS_ALIGNED(dst, 16) &&
      IS_ALIGNED(dst_stride, 16)) {
...
static int X420ToI420(const uint8* src_y, int src_stride_y0, int src_stride_y1,
                      const uint8* src_uv, int src_stride_uv, uint8* dst_y,
  ...
  void (*SplitUVRow)(const uint8* src_uv, uint8* dst_u, uint8* dst_v, int pix) =
      SplitUVRow_C;
  if (!src_y || !src_uv || !dst_y || !dst_u || !dst_v || width <= 0 ||
      height == 0) {
    return -1;
  }
  // Negative height means invert the image.
...
LIBYUV_API
int Q420ToI420(const uint8* src_y, int src_stride_y, const uint8* src_yuy2,
               int src_stride_yuy2, uint8* dst_y, int dst_stride_y,
  ...
  void (*YUY2ToYRow)(const uint8* src_yuy2, uint8* dst_y, int pix) =
      YUY2ToYRow_C;
  if (!src_y || !src_yuy2 || !dst_y || !dst_u || !dst_v || width <= 0 ||
      height == 0) {
    return -1;
  }
  // Negative height means invert the image.
...
int YUY2ToI420(const uint8* src_yuy2, int src_stride_yuy2, uint8* dst_y,
  ...
  void (*YUY2ToUVRow)(const uint8* src_yuy2, int src_stride_yuy2, uint8* dst_u,
                      uint8* dst_v, int pix) = YUY2ToUVRow_C;
  void (*YUY2ToYRow)(const uint8* src_yuy2, uint8* dst_y, int pix) =
      YUY2ToYRow_C;
  // Negative height means invert the image.
  if (height < 0) {
...
int UYVYToI420(const uint8* src_uyvy, int src_stride_uyvy, uint8* dst_y,
  ...
  void (*UYVYToUVRow)(const uint8* src_uyvy, int src_stride_uyvy, uint8* dst_u,
                      uint8* dst_v, int pix) = UYVYToUVRow_C;
...
LIBYUV_API
int BGRAToI420(const uint8* src_bgra, int src_stride_bgra, uint8* dst_y,
               int dst_stride_y, uint8* dst_u, int dst_stride_u, uint8* dst_v,
               int dst_stride_v, int width, int height) {
  int y;
...
LIBYUV_API
int ABGRToI420(const uint8* src_abgr, int src_stride_abgr, uint8* dst_y,
               int dst_stride_y, uint8* dst_u, int dst_stride_u, uint8* dst_v,
               int dst_stride_v, int width, int height) {
  int y;
...
LIBYUV_API
int RGBAToI420(const uint8* src_rgba, int src_stride_rgba, uint8* dst_y,
               int dst_stride_y, uint8* dst_u, int dst_stride_u, uint8* dst_v,
               int dst_stride_v, int width, int height) {
  int y;
...
LIBYUV_API
int RGB24ToI420(const uint8* src_rgb24, int src_stride_rgb24, uint8* dst_y,
                int dst_stride_y, uint8* dst_u, int dst_stride_u, uint8* dst_v,
                int dst_stride_v, int width, int height) {
  int y;
...
// Convert RAW to I420.
LIBYUV_API
int RAWToI420(const uint8* src_raw, int src_stride_raw, uint8* dst_y,
              int dst_stride_y, uint8* dst_u, int dst_stride_u, uint8* dst_v,
              int dst_stride_v, int width, int height) {
  int y;
  ...
  void (*RAWToYRow)(const uint8* src_raw, uint8* dst_y, int pix) = RAWToYRow_C;
#else
...
LIBYUV_API
int RGB565ToI420(const uint8* src_rgb565, int src_stride_rgb565, uint8* dst_y,
                 int dst_stride_y, uint8* dst_u, int dst_stride_u, uint8* dst_v,
                 int dst_stride_v, int width, int height) {
  int y;
