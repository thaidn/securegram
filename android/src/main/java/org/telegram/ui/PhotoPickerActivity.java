...
public class PhotoPickerActivity extends BaseFragment
  ...
  public PhotoPickerActivity(
      ...
      ArrayList<MediaController.SearchImage> recentImages,
  ...
  private void searchGiphyImages(String query, int offset, final int count) {
    ...
    try {
      ...
      String url =
          String.format(
              Locale.US,
              "https://api.giphy.com/v1/gifs/search?q=%s&offset=%d&limit=%d&api_key=141Wa2KDAfNfxu",
  ...
  private void fixLayoutInternal() {
    ...
    if (AndroidUtilities.isTablet()) {
      itemWidth =
          (AndroidUtilities.dp(490) - ((columnsCount + 1) * AndroidUtilities.dp(4))) / columnsCount;
    } else {
      itemWidth =
          (AndroidUtilities.displaySize.x - ((columnsCount + 1) * AndroidUtilities.dp(4)))
              / columnsCount;
    }
    ...
    if (selectedAlbum == null) {
      emptyView.setPadding(
          ...
          0,
          0,
          (int) ((AndroidUtilities.displaySize.y - ActionBar.getCurrentActionBarHeight()) * 0.4f));
    }
  }
