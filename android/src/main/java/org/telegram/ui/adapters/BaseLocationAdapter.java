    ...
    try {
      ...
      String url =
          String.format(
              ...
              BuildVars.FOURSQUARE_API_VERSION,
              BuildVars.FOURSQUARE_API_ID,
              BuildVars.FOURSQUARE_API_KEY,
              String.format(
                  Locale.US, "%f,%f", coordinate.getLatitude(), coordinate.getLongitude()));
      if (query != null && query.length() > 0) {
                    ...
                    for (int a = 0; a < result.length(); a++) {
                      try {
                        ...
                        if (location.has("address")) {
                          ...
                          venue.address =
                              String.format(Locale.US, "%f,%f", venue.geo.lat, venue.geo._long);
                        }
                        if (object.has("name")) {
