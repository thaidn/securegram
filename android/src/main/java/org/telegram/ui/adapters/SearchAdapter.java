...
public class SearchAdapter extends BaseSearchAdapter {
            ...
            Utilities.searchQueue.postRunnable(
                new Runnable() {
                  ...
                  public void run() {
                    ...
                    if (search1.length() == 0) {
                      updateSearchResults(
                          new ArrayList<TLRPC.User>(), new ArrayList<CharSequence>());
                      return;
                    }
  ...
  private void updateSearchResults(
      final ArrayList<TLRPC.User> users, final ArrayList<CharSequence> names) {
    AndroidUtilities.runOnUIThread(
        new Runnable() {
          @Override
          public void run() {
    ...
    if (i == searchResult.size()) {
      if (view == null) {
        ...
        ((GreySectionCell) view)
            .setText(LocaleController.getString("GlobalSearch", R.string.GlobalSearch));
      }
    } else {
      if (view == null) {
        if (useUserCell) {
      ...
      if (user != null) {
        ...
        if (useUserCell) {
          ...
          ((ProfileSearchCell) view).useSeparator =
              (i != getCount() - 1 && i != searchResult.size() - 1);
          if (ignoreUsers != null) {
