...
public class MentionsAdapter extends BaseSearchAdapter {
  ...
  public void searchUsernameOrHashtag(
      String text, int position, ArrayList<MessageObject> messageObjects) {
    ...
    if (foundType == 0) {
      ...
      for (TLRPC.TL_chatParticipant chatParticipant : info.participants) {
        ...
        if (user.username != null
            ...
            && (usernameString.length() > 0
                    && user.username.toLowerCase().startsWith(usernameString)
                || usernameString.length() == 0)) {
          newResult.add(user);
        }
      }
      ...
      for (HashtagObject hashtagObject : hashtags) {
        if (hashtagObject != null
            && hashtagObject.hashtag != null
            && hashtagObject.hashtag.startsWith(hashtagString)) {
      ...
      for (HashMap.Entry<Integer, TLRPC.BotInfo> entry : botInfo.entrySet()) {
        for (TLRPC.TL_botCommand botCommand : entry.getValue().commands) {
          if (botCommand != null
              ...
              && botCommand.command.startsWith(command)) {
    ...
    if (searchResultUsernames != null) {
      ...
      if (searchResultCommandsUsers != null && botsCount != 1) {
        return String.format(
            "%s@%s", searchResultCommands.get(i), searchResultCommandsUsers.get(i).username);
      }
