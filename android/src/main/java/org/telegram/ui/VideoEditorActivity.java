...
public class VideoEditorActivity extends BaseFragment
                ...
                new Runnable() {
                  ...
                  public void run() {
                    if (videoPlayer != null && videoPlayer.isPlaying()) {
                      ...
                      float progress =
                          (videoPlayer.getCurrentPosition() - startTime) / (endTime - startTime);
                      float lrdiff =
  ...
  public View createView(Context context) {
        ...
        new ActionBar.ActionBarMenuOnItemClick() {
          ...
          public void onItemClick(int id) {
            if (id == -1) {
              ...
              if (delegate != null) {
                if (compressVideo.getVisibility() == View.GONE
                    || compressVideo.getVisibility() == View.VISIBLE
                        && !compressVideo.isChecked()) {
                  delegate.didFinishEditVideo(
                      videoPath,
                      startTime,
                      endTime,
                      originalWidth,
    ...
    fragmentView =
        getParentActivity().getLayoutInflater().inflate(R.layout.video_editor_layout, null, false);
    ...
    compressVideo.setVisibility(
        originalHeight != resultHeight || originalWidth != resultWidth ? View.VISIBLE : View.GONE);
    compressVideo.setChecked(preferences.getBoolean("compress_video", true));
    compressVideo.setOnCheckedChangeListener(
    ...
    videoTimelineView.setDelegate(
        new VideoTimelineView.VideoTimelineViewDelegate() {
          @Override
    ...
    videoSeekBarView.delegate =
        new VideoSeekBarView.SeekBarDelegate() {
          @Override
  ...
  private void updateVideoOriginalInfo() {
    ...
    String videoTimeSize =
        String.format(
            "%d:%02d, %s", minutes, seconds, AndroidUtilities.formatFileSize(originalSize));
  ...
  private void updateVideoEditedInfo() {
    ...
    esimatedDuration =
        (long)
            Math.ceil(
                (videoTimelineView.getRightProgress() - videoTimelineView.getLeftProgress())
                    * videoDuration);
    ...
    if (compressVideo.getVisibility() == View.GONE
        || compressVideo.getVisibility() == View.VISIBLE && !compressVideo.isChecked()) {
    ...
    String videoTimeSize =
        String.format(
            "%d:%02d, ~%s", minutes, seconds, AndroidUtilities.formatFileSize(estimatedSize));
  ...
  private void fixVideoSize() {
    ...
    if (AndroidUtilities.isTablet()) {
      ...
      height =
          viewHeight
              - AndroidUtilities.dp(276 + (compressVideo.getVisibility() == View.VISIBLE ? 20 : 0));
    } else {
      if (getParentActivity().getResources().getConfiguration().orientation
        ...
        height =
            viewHeight
                - AndroidUtilities.dp(
                    276 + (compressVideo.getVisibility() == View.VISIBLE ? 20 : 0));
      }
    }
  ...
  private void fixLayoutInternal() {
    ...
    if (!AndroidUtilities.isTablet()
        && getParentActivity().getResources().getConfiguration().orientation
            == Configuration.ORIENTATION_LANDSCAPE) {
      FrameLayout.LayoutParams layoutParams =
          (FrameLayout.LayoutParams) videoContainerView.getLayoutParams();
      ...
      layoutParams.bottomMargin =
          AndroidUtilities.dp(260 + (compressVideo.getVisibility() == View.VISIBLE ? 20 : 0));
      ...
      layoutParams.leftMargin = 0;
      layoutParams.bottomMargin =
          AndroidUtilities.dp(150 + (compressVideo.getVisibility() == View.VISIBLE ? 20 : 0));
      layoutParams.width = LayoutHelper.MATCH_PARENT;
