...
public class PhotoAlbumPickerActivity extends BaseFragment
  ...
  private void openPhotoPicker(MediaController.AlbumEntry albumEntry, int type) {
    ...
    fragment.setDelegate(
        new PhotoPickerActivity.PhotoPickerActivityDelegate() {
          @Override
  ...
  private class ListAdapter extends BaseFragmentAdapter {
    ...
    public int getCount() {
      if (singlePhoto || selectedMode == 0) {
        if (singlePhoto) {
          return albumsSorted != null
              ? (int) Math.ceil(albumsSorted.size() / (float) columnsCount)
              : 0;
        }
        return 1
            + (albumsSorted != null
        ...
        return (videoAlbumsSorted != null
            ? (int) Math.ceil(videoAlbumsSorted.size() / (float) columnsCount)
            : 0);
      }
    }
      ...
      if (type == 0) {
        ...
        if (view == null) {
          ...
          ((PhotoPickerSearchCell) view)
              .setDelegate(
                  new PhotoPickerSearchCell.PhotoPickerSearchCellDelegate() {
                    @Override
