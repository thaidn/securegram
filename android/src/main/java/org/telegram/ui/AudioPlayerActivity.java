...
public class AudioPlayerActivity extends BaseFragment
    implements NotificationCenter.NotificationCenterDelegate,
        MediaController.FileDownloadProgressListener {
  ...
  private class SeekBarView extends FrameLayout {
    ...
    boolean onTouch(MotionEvent ev) {
      if (ev.getAction() == MotionEvent.ACTION_DOWN) {
        ...
        if (thumbX - additionWidth <= ev.getX()
            && ev.getX() <= thumbX + thumbWidth + additionWidth
            && ev.getY() >= 0
            && ev.getY() <= getMeasuredHeight()) {
          pressed = true;
    ...
    protected void onDraw(Canvas canvas) {
      ...
      canvas.drawRect(
          thumbWidth / 2,
          getMeasuredHeight() / 2 - AndroidUtilities.dp(1),
          getMeasuredWidth() - thumbWidth / 2,
          getMeasuredHeight() / 2 + AndroidUtilities.dp(1),
          innerPaint1);
      canvas.drawRect(
          thumbWidth / 2,
      ...
      canvas.drawCircle(
          thumbX + thumbWidth / 2,
          y + thumbHeight / 2,
          AndroidUtilities.dp(pressed ? 8 : 6),
          outerPaint1);
    }
  }
  ...
  public View createView(Context context) {
    ...
    frameLayout.addView(
        shadow,
        LayoutHelper.createFrame(
            LayoutHelper.MATCH_PARENT, 3, Gravity.BOTTOM | Gravity.LEFT, 0, 0, 0, 96));
    ...
    frameLayout.addView(
        seekBarContainer,
        LayoutHelper.createFrame(
            LayoutHelper.MATCH_PARENT, 30, Gravity.BOTTOM | Gravity.LEFT, 0, 0, 0, 66));
    ...
    seekBarContainer.addView(
        timeTextView,
        LayoutHelper.createFrame(44, LayoutHelper.MATCH_PARENT, Gravity.TOP | Gravity.LEFT));
    ...
    seekBarContainer.addView(
        durationTextView,
        LayoutHelper.createFrame(44, LayoutHelper.MATCH_PARENT, Gravity.TOP | Gravity.RIGHT));
    ...
    seekBarContainer.addView(
        progressView,
        LayoutHelper.createFrame(
            LayoutHelper.MATCH_PARENT, 2, Gravity.CENTER_VERTICAL | Gravity.LEFT, 44, 0, 44, 0));
        ...
        new FrameLayout(context) {
          ...
          protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            ...
            for (int a = 0; a < 5; a++) {
              ...
              buttons[a]
                  .layout(
                      l, t, l + buttons[a].getMeasuredWidth(), t + buttons[a].getMeasuredHeight());
            }
          }
        };
    ...
    frameLayout.addView(
        bottomView,
        LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 66, Gravity.BOTTOM | Gravity.LEFT));
          ...
          public void onClick(View v) {
            ...
            if (MediaController.getInstance().isAudioPaused()) {
              MediaController.getInstance()
                  .playAudio(MediaController.getInstance().getPlayingMessageObject());
            } else {
              MediaController.getInstance()
  ...
  public void didReceivedNotification(int id, Object... args) {
    if (id == NotificationCenter.audioDidStarted
        || id == NotificationCenter.audioPlayStateChanged
        || id == NotificationCenter.audioDidReset) {
  ...
  private void updateProgress(MessageObject messageObject) {
    if (seekBarView != null) {
      ...
      String timeString =
          String.format(
              "%d:%02d", messageObject.audioProgressSec / 60, messageObject.audioProgressSec % 60);
  ...
  private void updateTitle(boolean shutdown) {
    ...
    if (messageObject == null && shutdown || messageObject != null && !messageObject.isMusic()) {
      if (!parentLayout.fragmentsStack.isEmpty()
          && parentLayout.fragmentsStack.get(parentLayout.fragmentsStack.size() - 1) == this) {
        finishFragment();
      } else {
        removeSelfFromStack();
      }
