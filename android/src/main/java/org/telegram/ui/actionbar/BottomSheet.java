...
public class BottomSheet extends Dialog {
  ...
  private static class BottomSheetCell extends FrameLayout {
    ...
    public BottomSheetCell(Context context, boolean grid) {
      ...
      if (grid) {
        addView(
            imageView,
            LayoutHelper.createFrame(48, 48, Gravity.CENTER_HORIZONTAL | Gravity.TOP, 0, 8, 0, 0));
      } else {
        addView(
            imageView,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
      super.onMeasure(
          isGrid
              ? MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(96), MeasureSpec.EXACTLY)
              : widthMeasureSpec,
          MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(isGrid ? 80 : 48), MeasureSpec.EXACTLY));
    }
    ...
    public void setTextAndIcon(CharSequence text, int icon) {
      ...
      if (icon != 0) {
        ...
        if (!isGrid) {
          textView.setPadding(
              LocaleController.isRTL ? 0 : AndroidUtilities.dp(56),
              0,
              LocaleController.isRTL ? AndroidUtilities.dp(56) : 0,
              0);
        }
      } else {
        ...
        new FrameLayout(getContext()) {
          ...
          protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            if (containerView != null) {
              int left =
                  useRevealAnimation && Build.VERSION.SDK_INT <= 19 ? 0 : backgroundPaddingLeft;
              if (!fullWidth) {
                if (AndroidUtilities.isTablet()) {
                  ...
                  containerView.measure(
                      MeasureSpec.makeMeasureSpec(side + left * 2, MeasureSpec.EXACTLY),
                      MeasureSpec.makeMeasureSpec(height, MeasureSpec.AT_MOST));
                } else {
                  int maxWidth = Math.min(AndroidUtilities.dp(480), width);
                  containerView.measure(
                      isPortrait
                          ? MeasureSpec.makeMeasureSpec(width + left * 2, MeasureSpec.EXACTLY)
                          : MeasureSpec.makeMeasureSpec(
                              (int) Math.max(width * 0.8f, maxWidth) + left * 2,
                              MeasureSpec.EXACTLY),
                      MeasureSpec.makeMeasureSpec(height, MeasureSpec.AT_MOST));
                }
              } else {
          ...
          protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            if (containerView != null) {
              ...
              containerView.layout(
                  l, t, l + containerView.getMeasuredWidth(), t + getMeasuredHeight());
            }
          }
        };
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (items != null) {
      if (customView != null) {
        ...
        containerView.addView(
            frameLayout, LayoutHelper.createLinear(LayoutHelper.MATCH_PARENT, 16));

        View lineView = new View(getContext());
        lineView.setBackgroundColor(0xffd2d2d2);
        frameLayout.addView(
            lineView, new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 1));
      }
          ...
          if (rowLayout == null || lastRowLayoutNum != row) {
            ...
            containerView.addView(
                ...
                LayoutHelper.createLinear(
                    LayoutHelper.MATCH_PARENT, 80, 0, lastRowLayoutNum != 0 ? 8 : 0, 0, 0));
            rowLayout.setOnTouchListener(
                new View.OnTouchListener() {
                  @Override
                  public boolean onTouch(View v, MotionEvent event) {
                    return true;
  ...
  public void show() {
    ...
    containerView.setPadding(
        left,
        (applyTopPaddings ? AndroidUtilities.dp(8) : 0) + top,
        left,
        (applyTopPaddings ? AndroidUtilities.dp(isGrid ? 16 : 8) : 0));
    if (Build.VERSION.SDK_INT < 21) {
      startOpenAnimation();
    }
  }
      ...
      finalRevealRadius =
          Math.max(
              ...
              (int)
                  Math.ceil(
                      Math.sqrt(
                          (revealX - corners[a][0]) * (revealX - corners[a][0])
                              + (revealY - corners[a][1]) * (revealY - corners[a][1]))));
    }

    ArrayList<Animator> animators = new ArrayList<>(3);
    animators.add(
        ObjectAnimator.ofFloat(
            this, "revealRadius", open ? 0 : finalRevealRadius, open ? finalRevealRadius : 0));
    ...
    if (containerView.getMeasuredHeight() == 0) {
      containerView.measure(
          View.MeasureSpec.makeMeasureSpec(
              AndroidUtilities.displaySize.x, View.MeasureSpec.AT_MOST),
          View.MeasureSpec.makeMeasureSpec(
              AndroidUtilities.displaySize.y, View.MeasureSpec.AT_MOST));
    }
  ...
  public static class Builder {
    ...
    public Builder setItems(
        CharSequence[] items, int[] icons, final OnClickListener onClickListener) {
      bottomSheet.items = items;
