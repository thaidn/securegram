...
public class DrawerLayoutContainer extends FrameLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (applyInsets) {
                if (Build.VERSION.SDK_INT >= 16 && child.getFitsSystemWindows()) {
            ...
            if (drawerLayout != child) {
                ...
                final int drawerWidthSpec =
                    getChildMeasureSpec(
                        widthMeasureSpec, minDrawerMargin + lp.leftMargin + lp.rightMargin,
                        lp.width);
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (scrimOpacity > 0 && drawingContent) {
            ...
            final float alpha = Math.max(0,
                Math.min(drawerPosition / AndroidUtilities.dp(20), 1.0f));
            if (alpha != 0) {
                shadowLeft.setBounds(
                    (int) drawerPosition,
                    child.getTop(),
                    (int) drawerPosition + shadowLeft.getIntrinsicWidth(),
                    child.getBottom());
