...
public class LetterSectionsListView extends ListView implements AbsListView.OnScrollListener {
  ...
  public void onScroll(
    ...
    if (currentFirst != firstVisibleItem || currentVisible != visibleItemCount) {
      ...
      int itemNum =
          firstVisibleItem
              + mAdapter.getCountForSection(startSection)
              - mAdapter.getPositionInSectionForPosition(firstVisibleItem);
      while (true) {
    ...
    for (View header : headers) {
      ...
      if (top < 0) {
        canvas.saveLayerAlpha(
            ...
            (int) (255 * (1.0f + (float) top / (float) header.getMeasuredHeight())),
