...
public class PopupNotificationActivity extends Activity
  ...
  private class FrameLayoutTouch extends FrameLayoutFixed {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
      return checkTransitionAnimation()
          || ((PopupNotificationActivity) getContext()).onTouchEventMy(ev);
    }
  ...
  private void switchToNextMessage() {
    if (NotificationsController.getInstance().popupMessages.size() > 1) {
      ...
      currentMessageObject =
          NotificationsController.getInstance().popupMessages.get(currentMessageNum);
      updateInterfaceForCurrentMessage(2);
      countText.setText(
          String.format(
              "%d/%d",
              currentMessageNum + 1,
              NotificationsController.getInstance().popupMessages.size()));
    }
  }
    ...
    if (motionEvent != null && motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
      ...
      if (motionEvent != null && startedMoving) {
        FrameLayout.LayoutParams layoutParams =
            (FrameLayout.LayoutParams) centerView.getLayoutParams();
  ...
  private ViewGroup getViewForMessage(int num, boolean applyOffset) {
    ...
    if (messageObject.type == MessageObject.Type.PHOTO ||
        messageObject.type == MessageObject.Type.LOCATION) {
      ...
      } else if (messageObject.type == MessageObject.Type.LOCATION) {
        messageText.setVisibility(View.GONE);
        messageText.setText(messageObject.messageText);
        imageView.setVisibility(View.VISIBLE);
    ...
    } else if (messageObject.type == MessageObject.Type.AUDIO) {
      PopupAudioView cell;
