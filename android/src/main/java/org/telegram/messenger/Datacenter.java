...
public class Datacenter {
  ...
  public void replaceAddressesAndPorts(
      ArrayList<String> newAddresses, HashMap<String, Integer> newPorts, int flags) {
    ArrayList<String> addresses;
    if ((flags & 2) != 0) {
      if ((flags & 1) != 0) {
        addresses = addressesIpv6Download;
  ...
  public long selectServerSalt(int date) {
    ...
    for (ServerSalt salt : authServerSaltSet) {
      if (salt.validUntil < date
          || (salt.validSince == 0 && salt.validUntil == Integer.MAX_VALUE)) {
        cleanupNeeded = true;
