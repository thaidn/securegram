...
public class ConnectionsManager
    implements Action.ActionDelegate, TcpConnection.TcpConnectionDelegate {
  ...
  private final HashMap<Integer, ArrayList<NetworkMessage>> genericMessagesToDatacenters =
      new HashMap<>();
      ...
      new Runnable() {
        ...
        public void run() {
          ...
          if (datacenters != null) {
            ...
            if (sendingPushPing && lastPushPingTime < System.currentTimeMillis() - 30000
                || Math.abs(lastPushPingTime - System.currentTimeMillis()) > 60000 * 3 + 10000) {
              lastPushPingTime = 0;
          ...
          if (lastPauseTime != 0 && lastPauseTime < currentTime - nextSleepTimeout) {
            ...
            if (!dontSleep) {
              for (int a = 0; a < runningRequests.size(); a++) {
                ...
                if (request.rawRequest instanceof TLRPC.TL_get_future_salts) {
                  dontSleep = true;
                } else if (request.retryCount < 10
                    && (request.runningStartTime + 60 > (int) (currentTime / 1000))
                    && ((request.flags & RPCRequest.RPCRequestClassDownloadMedia) != 0
                        || (request.flags & RPCRequest.RPCRequestClassUploadMedia) != 0)) {
                  dontSleep = true;
                  break;
                }
              }
            ...
            if (!dontSleep) {
              if (!paused) {
                FileLog.e(
                    "tmessages", "pausing network and timers by sleep time = " + nextSleepTimeout);
              ...
              FileLog.e(
                  "tmessages",
                  "don't sleep 30 seconds because of salt, upload or download request");
            }
          }
          if (paused) {
            paused = false;
            ...
            if (datacenter != null) {
              if (datacenter.authKey != null) {
                ...
                if (!updatingDcSettings
                    && lastDcUpdateTime
                        < (int) (System.currentTimeMillis() / 1000) - DC_UPDATE_TIME) {
                  updateDcSettings(0);
                }
  ...
  private void loadSession() {
    Utilities.stageQueue.postRunnable(
        new Runnable() {
          ...
          public void run() {
            File configFile =
                new File(ApplicationLoader.applicationContext.getFilesDir(), "config.dat");
            ...
            try {
              ...
              if (currentDatacenter != null) {
                ...
                if (!datacenters.isEmpty()) {
                  ...
                  editor.putString(
                      "datacenters", Base64.encodeToString(data.toByteArray(), Base64.DEFAULT));
                  data.cleanup();
                } else {
  ...
  void clearRequestsForRequestClass(int requestClass, Datacenter datacenter) {
    for (int a = 0; a < runningRequests.size(); a++) {
      ...
      if ((request.flags & requestClass) != 0
          && dcenter != null
          && dcenter.datacenterId == datacenter.datacenterId) {
  ...
  long generateMessageId() {
    long messageId =
        (long)
            ((((double) System.currentTimeMillis() + ((double) timeDifference) * 1000)
                    * 4294967296.0)
                / 1000.0);
            ...
            new RPCRequest.RPCRequestDelegate() {
              ...
              public void run(TLObject response, TLRPC.TL_error error) {
                ...
                if (error == null) {
                  ...
                  lastDcUpdateTime =
                      (int) (System.currentTimeMillis() / 1000) - DC_UPDATE_TIME + updateIn;
                  ...
                  for (int a = 0; a < config.dc_options.size(); a++) {
                    ...
                    existing.addAddressAndPort(
                        datacenterDesc.ip_address, datacenterDesc.port, datacenterDesc.flags);
                  }
                  ...
                  if (!datacentersArr.isEmpty()) {
                    for (int a = 0; a < datacentersArr.size(); a++) {
                      ...
                      if (exist == null) {
                        ...
                        exist.replaceAddressesAndPorts(
                            datacenter.addressesIpv4, datacenter.ports, 0);
                        exist.replaceAddressesAndPorts(
                            datacenter.addressesIpv6, datacenter.ports, 1);
                        exist.replaceAddressesAndPorts(
                            datacenter.addressesIpv4Download, datacenter.ports, 2);
                        exist.replaceAddressesAndPorts(
                            datacenter.addressesIpv6Download, datacenter.ports, 3);
                      }
  ...
  private TLObject wrapInLayer(TLObject object, int datacenterId, RPCRequest request) {
    if (object.layer() > 0) {
      ...
      if (datacenter == null || datacenter.lastInitVersion != BuildVars.BUILD_VERSION) {
        ...
        try {
          invoke.lang_code =
              LocaleController.getLocaleString(
                  LocaleController.getInstance().getSystemDefaultLocale());
  ...
  public long performRpc(final TLObject rpc, final RPCRequest.RPCRequestDelegate completionBlock) {
    return performRpc(
        rpc, completionBlock, null, true, RPCRequest.RPCRequestClassGeneric, DEFAULT_DATACENTER_ID);
  }

  public long performRpc(
      final TLObject rpc,
      final RPCRequest.RPCRequestDelegate completionBlock,
      boolean requiresCompletion,
      int requestClass) {
    return performRpc(
        rpc, completionBlock, null, requiresCompletion, requestClass, DEFAULT_DATACENTER_ID, true);
  }
  ...
  public long performRpc(
      ...
      final boolean requiresCompletion,
      final int requestClass,
      final int datacenterId) {
    return performRpc(
        rpc, completionBlock, quickAckBlock, requiresCompletion, requestClass, datacenterId, true);
  }
  ...
  public long performRpc(
      ...
      final int requestClass,
      final int datacenterId,
      final boolean runQueue) {
    if (rpc == null
        || !UserConfig.isClientActivated()
            && (requestClass & RPCRequest.RPCRequestClassWithoutLogin) == 0) {
            ...
            if (!ifNotSent) {
              for (int i = 0; i < runningRequests.size(); i++) {
                ...
                if (request.token == token) {
                  ...
                  FileLog.d(
                      "tmessages", "===== Cancelled running rpc request " + request.rawRequest);
  ...
  private void processRequestQueue(int requestClass, int _datacenterId) {
    ...
    for (int i = 0; i < runningRequests.size(); i++) {
      ...
      if (datacenters.size() > 1
          && (request.flags & RPCRequest.RPCRequestClassTryDifferentDc) != 0) {
      ...
      if (!request.initRequest && requestDatacenter.lastInitVersion != BuildVars.BUILD_VERSION) {
        request.rpcRequest =
            wrapInLayer(request.rawRequest, requestDatacenter.datacenterId, request);
      ...
      if (requestDatacenter == null) {
        ...
        continue;
      } else if (!requestDatacenter.authorized
          && request.runningDatacenterId != DEFAULT_DATACENTER_ID
          && request.runningDatacenterId != currentDatacenterId
          && (request.flags & RPCRequest.RPCRequestClassEnableUnauthorized) == 0) {
      ...
      boolean forceThisRequest =
          (request.flags & requestClass) != 0 && requestDatacenter.datacenterId == _datacenterId;

      if (request.rawRequest instanceof TLRPC.TL_get_future_salts
          || request.rawRequest instanceof TLRPC.TL_destroy_session) {
      ...
      if (((Math.abs(currentTime - request.runningStartTime) > maxTimeout)
              && (currentTime > request.runningMinStartTime
                  || Math.abs(currentTime - request.runningMinStartTime) > 60.0))
          || forceThisRequest) {
        if (!forceThisRequest && request.transportChannelToken > 0) {
          if ((request.flags & RPCRequest.RPCRequestClassGeneric) != 0
            ...
            if (connection.channelToken != 0
              ...
              FileLog.d(
                  "tmessages",
                  "Request download token is valid, not retrying " + request.rawRequest);
              continue;
            }
          }
        }

        if (request.transportChannelToken != 0
            && request.transportChannelToken != connection.channelToken) {
    ...
    if (genericConnection != null && genericConnection.channelToken != 0) {
      ...
      for (int a = 0; a < sessionsToDestroy.size(); a++) {
        ...
        if (System.currentTimeMillis() / 1000 - lastDestroySessionRequestTime > 2.0) {
          ...
          NetworkMessage networkMessage = new NetworkMessage();
          networkMessage.protoMessage =
              wrapMessage(destroySession, currentDatacenter.connection, false);
    ...
    for (Datacenter datacenter : datacenters.values()) {
      if (genericMessagesToDatacenters.get(datacenter.datacenterId) == null
          && datacenter.connection != null
          && datacenter.connection.channelToken != 0
          && datacenter.connection.hasMessagesToConfirm()) {
    ...
    for (int iter : genericMessagesToDatacenters.keySet()) {
      ...
      if (datacenter != null) {
        ...
        for (int b = 0; b < arr.size(); b++) {
          ...
          if (rawRequest != null
              && (rawRequest instanceof TLRPC.TL_messages_sendMessage
    ...
    for (int a = 0; a < unauthorizedDatacenterIds.size(); a++) {
      ...
      if (num != currentDatacenterId
          && num != movingToDatacenterId
          && UserConfig.isClientActivated()) {
        boolean notFound = true;
        for (int b = 0; b < actionQueue.size(); b++) {
          Action actor = actionQueue.get(b);
  ...
  TLRPC.TL_protoMessage wrapMessage(
      TLObject message, TcpConnection connection, boolean meaningful) {
    ByteBufferDesc os = new ByteBufferDesc(true);
  ...
  void proceedToSendingMessages(
      ArrayList<NetworkMessage> messageList, TcpConnection connection, boolean reportAck) {
  ...
  void sendMessagesToTransport(
      ArrayList<NetworkMessage> messagesToSend, TcpConnection connection, boolean reportAck) {
    ...
    for (int a = 0; a < messagesToSend.size(); a++) {
      ...
      if (currentSize >= 3 * 1024 || a == messagesToSend.size() - 1) {
        ...
        ByteBufferDesc transportData =
            createConnectionData(currentMessages, quickAckId, connection);
  ...
  ByteBufferDesc createConnectionData(
      ArrayList<NetworkMessage> messages, ArrayList<Integer> quickAckId, TcpConnection connection) {
        ...
        if (message.body instanceof TLRPC.invokeWithLayer) {
          FileLog.d(
              ...
              connection.getSissionId()
                  ...
                  + message.msg_id
                  + "): "
                  + ((TLRPC.invokeWithLayer) message.body).query);
          ...
          if (r.query instanceof TLRPC.invokeWithLayer) {
            FileLog.d(
                ...
                connection.getSissionId()
                    ...
                    + message.msg_id
                    + "): "
                    + ((TLRPC.invokeWithLayer) r.query).query);
          } else {
            FileLog.d(
                "tmessages",
                connection.getSissionId()
    ...
    ByteBufferDesc innerOs =
        BuffersStorage.getInstance().getFreeBuffer(8 + 8 + 8 + 4 + 4 + sizeBuffer.length());
    ...
    MessageKeyData keyData =
        MessageKeyData.generateMessageKeyData(datacenter.authKey, messageKey, false);
    ...
    ByteBufferDesc dataForEncryption =
        BuffersStorage.getInstance().getFreeBuffer(innerOs.limit() + zeroCount);
    ...
    Utilities.aesIgeEncryption(
        dataForEncryption.buffer,
        keyData.aesKey,
        keyData.aesIv,
        true,
        false,
        0,
        dataForEncryption.limit());

    ByteBufferDesc data =
        BuffersStorage.getInstance()
            .getFreeBuffer(8 + messageKey.length + dataForEncryption.limit());
        ...
        new Runnable() {
          ...
          public void run() {
            ...
            performRpc(
                ...
                null,
                true,
                RPCRequest.RPCRequestClassGeneric | RPCRequest.RPCRequestClassWithoutLogin,
                datacenter.datacenterId);
          }
        });
  }
    ...
    if (message instanceof TLRPC.TL_new_session_created) {
      ...
      if (!connection.isSessionProcessed(newSession.unique_id)) {
        ...
        for (int a = 0; a < runningRequests.size(); a++) {
          RPCRequest request = runningRequests.get(a);
          Datacenter dcenter = datacenterWithId(request.runningDatacenterId);
          if (request.runningMessageId < newSession.first_msg_id
              && (request.flags & connection.transportRequestClass) != 0
              && dcenter != null
              && dcenter.datacenterId == datacenter.datacenterId) {
            request.runningMessageId = 0;
            request.runningMessageSeqNo = 0;
      ...
      if (UserConfig.isClientActivated()
          && !UserConfig.registeredForInternalPush
          && (connection.transportRequestClass & RPCRequest.RPCRequestClassPush) != 0) {
        registerForPush();
      }
      if ((connection.transportRequestClass & RPCRequest.RPCRequestClassPush) == 0) {
      ...
      for (int a = 0; a < lst.size(); a++) {
        ...
        if (session == res.session_id) {
          ...
          FileLog.d(
              "tmessages",
              String.format(
                  "Destroyed session %d (%s)",
                  res.session_id,
                  res instanceof TLRPC.TL_destroy_session_ok ? "ok" : "not found"));
          break;
        }
      }
      ...
      if (resultContainer.result instanceof TLRPC.RpcError) {
        ...
        FileLog.e(
            ...
            String.format(
                "***** RPC error %d: %s",
                ((TLRPC.RpcError) resultContainer.result).error_code,
                errorMessage));
            ...
            if (request.completionBlock != null) {
              ...
              if (resultContainer.result instanceof TLRPC.TL_gzip_packed) {
                ...
                resultContainer.result =
                    Utilities.decompress(packet.packed_data, request.rawRequest, true);
              }
              if (resultContainer.result instanceof TLRPC.RpcError) {
                ...
                if (errorCode == 500 || errorCode < 0) {
                  ...
                  if ((request.flags & RPCRequest.RPCRequestClassFailOnServerErrors) == 0) {
                    ...
                    discardResponse = true;
                    request.wait = true;
                    request.runningMinStartTime =
                        (int) (System.currentTimeMillis() / 1000 + waitTime);
                    request.confirmed = false;
                  }
                }
                ...
                if (request.rawRequest == null || resultContainer.result == null) {
                  ...
                  if (request.rawRequest == null) {
                    ...
                    FileLog.e(
                        "tmessages",
                        "***** RPC error: invalid response class "
                            + resultContainer.result
                            + " (for request "
                            + request.rawRequest
                            + ")");
              ...
              if (!discardResponse) {
                if (implicitError != null || resultContainer.result instanceof TLRPC.TL_error) {
                  isError = true;
                  request.completionBlock.run(
                      null,
                      implicitError != null
                          ? implicitError
                          : (TLRPC.TL_error) resultContainer.result);
                } else {
              ...
              if (implicitError != null && implicitError.code == 401) {
                allowInitConnection = false;
                isError = true;
                if (implicitError.text != null
                    && implicitError.text.contains("SESSION_PASSWORD_NEEDED")) {
                  //ignore this error
                } else if (datacenter.datacenterId == currentDatacenterId
                    || datacenter.datacenterId == movingToDatacenterId) {
                  if ((request.flags & RPCRequest.RPCRequestClassGeneric) != 0
                      && UserConfig.isClientActivated()) {
                    UserConfig.clearConfig();
                    AndroidUtilities.runOnUIThread(
      ...
      if (badMsgNotification.error_code == 16
          ...
          || badMsgNotification.error_code == 64) {
        ...
        for (int a = 0; a < runningRequests.size(); a++) {
          ...
          if (request.respondsToMessageId(detailedInfo.msg_id)) {
            ...
            if (request.lastResendTime == 0
                || request.lastResendTime + 60 < (int) (System.currentTimeMillis() / 1000)) {
      ...
      TLObject result =
          Utilities.decompress(packet.packed_data, getRequestWithMessageId(messageId), true);
      if (result != null) {
        processMessage(
            result,
            messageId,
  ...
  public void tcpConnectionClosed(TcpConnection connection) {
    if (connection.getDatacenterId() == currentDatacenterId
        && (connection.transportRequestClass & RPCRequest.RPCRequestClassGeneric) != 0) {
      if (isNetworkOnline()) {
        connectionState = 2;
      } else {
        connectionState = 1;
  ...
  private void finishUpdatingState(TcpConnection connection) {
    if (connection.getDatacenterId() == currentDatacenterId
      ...
      if (ConnectionsManager.getInstance().connectionState == 3
          && !MessagesController.getInstance().gettingDifference
          && !MessagesController.getInstance().gettingDifferenceAgain) {
  ...
  public void tcpConnectionReceivedData(TcpConnection connection, ByteBufferDesc data, int length) {
    ...
    if (keyId == 0) {
      ...
      MessageKeyData keyData =
          MessageKeyData.generateMessageKeyData(datacenter.authKey, messageKey, true);

      Utilities.aesIgeEncryption(
          data.buffer, keyData.aesKey, keyData.aesIv, false, false, data.position(), length - 24);
      ...
      if (messageSessionId != connection.getSissionId()) {
        FileLog.e(
            "tmessages",
            String.format(
                "***** Error: invalid message session ID (%d instead of %d)",
                messageSessionId,
                connection.getSissionId()));
        finishUpdatingState(connection);
        return;
      ...
      byte[] realMessageKeyFull =
          Utilities.computeSHA1(data.buffer, 24, Math.min(messageLength + 32 + 24, data.limit()));
      if (realMessageKeyFull == null) {
        return;
      }

      if (!Utilities.arraysEquals(
          messageKey, 0, realMessageKeyFull, realMessageKeyFull.length - 16)) {
    ...
    if (message == null) {
      if (request != null) {
        ...
        if (message == null) {
          FileLog.e(
              "tmessages",
              String.format(Locale.US, "***** Error parsing message: %x", constructor));
        }
      } else {
        FileLog.d(
            "tmessages",
            String.format(Locale.US, "***** Not found request to parse message: %x", constructor));
      }
    } else if (message instanceof TLRPC.TL_rpc_result
        && ((TLRPC.TL_rpc_result) message).result == null) {
      message = null;
    }
    return message;
  }
  ...
  void moveToDatacenter(final int datacenterId) {
    ...
    if (UserConfig.isClientActivated()) {
      TLRPC.TL_auth_exportAuthorization exportAuthorization =
          new TLRPC.TL_auth_exportAuthorization();
  ...
  void authorizeOnMovingDatacenter() {
    ...
    if (movingAuthorization != null) {
      TLRPC.TL_auth_importAuthorization importAuthorization =
          new TLRPC.TL_auth_importAuthorization();
  ...
  public void ActionDidFinishExecution(final Action action, HashMap<String, Object> params) {
    if (action instanceof HandshakeAction) {
      ...
      if (eactor.datacenter.datacenterId == currentDatacenterId
          || eactor.datacenter.datacenterId == movingToDatacenterId) {
