...
public class HandshakeAction extends Action implements TcpConnection.TcpConnectionDelegate {
  ...
  public void execute(HashMap params) {
    FileLog.d(
        "tmessages",
        String.format(Locale.US, "Begin handshake with DC%d", datacenter.datacenterId));
    beginHandshake(true);
  }
    ...
    synchronized (lock) {
      if (serverPublicKeys == null) {
        ...
        map.put(
            ...
            new BigInteger[] {
              new BigInteger(
                  "c150023e2f70db7985ded064759cfecf0af328e69a41daf4d6f01b538135"
                      ...
                      + "31b32eee64ad15a8ba68885cde74a5bfc920f6abf59ba5c75506373e7130f9042da922179251f",
                  16),
        ...
        map.put(
            ...
            new BigInteger[] {
              new BigInteger(
                  "c6aeda78b02a251db4b6441031f467fa871faed32526c436524b1fb3b5dc"
        ...
        map.put(
            ...
            new BigInteger[] {
              new BigInteger(
                  "b1066749655935f0a5936f517034c943bea7f3365a8931ae52c8bcb14856"
                      ...
                      + "669a6da520859fb6d6c6203dfdfc79c75ec3ee97da8774b2da903e3435f2cd294670a75a526c1",
                  16),
              new BigInteger("010001", 16)
            });
        ...
        map.put(
            ...
            new BigInteger[] {
              new BigInteger(
                  "c2a8c55b4a62e2b78a19b91cf692bcdc4ba7c23fe4d06f194e2a0c30f6d9"
  ...
  ByteBufferDesc sendMessageData(TLObject message, long messageId) {
    ...
    ByteBufferDesc messageOs =
        BuffersStorage.getInstance().getFreeBuffer(8 + 8 + 4 + messageLength);
  ...
  void processMessage(TLObject message, long messageId) {
    if (message instanceof TLRPC.TL_resPQ) {
        ...
        new Thread(
                new Runnable() {
                  ...
                  public void run() {
                    ...
                    final Utilities.TPFactorizedValue factorizedPq =
                        Utilities.getFactorizedValue(pqf);
                          ...
                          public void run() {
                            ...
                            byte[] encryptedBytes =
                                Utilities.encryptWithRSA(
                                    (BigInteger[]) publicKey.get("key"),
                                    dataWithHash.toByteArray());
      ...
      if (message instanceof TLRPC.TL_server_DH_params_ok) {
        ...
        ByteBufferDesc answerWithHash =
            BuffersStorage.getInstance().getFreeBuffer(serverDhParams.encrypted_answer.length);
        answerWithHash.writeRaw(serverDhParams.encrypted_answer);
        answerWithHash.position(0);

        Utilities.aesIgeEncryption(
            answerWithHash.buffer,
            tmpAesKey.toByteArray(),
            tmpAesIv.toByteArray(),
            false,
            false,
            0,
            serverDhParams.encrypted_answer.length);
        byte[] answerHash = new byte[20];
        answerWithHash.readRaw(answerHash, false);

        boolean hashVerified = false;
        for (int i = 0; i < 16; i++) {
          byte[] computedAnswerHash =
              Utilities.computeSHA1(answerWithHash.buffer, 20, answerWithHash.limit() - i);
        ...
        TLRPC.TL_server_DH_inner_data dhInnerData =
            TLRPC.TL_server_DH_inner_data.TLdeserialize(answerWithHash, constructor, false);
        BuffersStorage.getInstance().reuseFreeBuffer(answerWithHash);
        ...
        FileLog.d(
            "tmessages", String.format(Locale.US, "===== Time difference: %d", timeDifference));
        ...
        ByteBufferDesc os =
            BuffersStorage.getInstance().getFreeBuffer(clientInnerData.getObjectSize());
        clientInnerData.serializeToStream(os);

        int len = os.length() + 20;
        int extraLen = len % 16 != 0 ? 16 - len % 16 : 0;
        ByteBufferDesc clientDataWithHash =
            BuffersStorage.getInstance().getFreeBuffer(len + extraLen);
        ...
        Utilities.aesIgeEncryption(
            clientDataWithHash.buffer,
            tmpAesKey.toByteArray(),
            tmpAesIv.toByteArray(),
            true,
            false,
            0,
            clientDataWithHash.length());
      ...
      if (message instanceof TLRPC.TL_dh_gen_ok) {
        ...
        FileLog.d(
            "tmessages", String.format("Handshake with DC%d completed", datacenter.datacenterId));
