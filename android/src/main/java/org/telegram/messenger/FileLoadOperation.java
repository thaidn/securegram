...
public class FileLoadOperation {
  ...
  private static final int downloadChunkSize = 1024 * 32;
  private static final int downloadChunkSizeBig = 1024 * 128;
  ...
  public void start() {
    ...
    currentDownloadChunkSize =
        totalBytesCount >= 1024 * 1024 * 30 ? downloadChunkSizeBig : downloadChunkSize;
    ...
    if (location.volume_id != 0 && location.local_id != 0) {
      ...
      if (datacenter_id == Integer.MIN_VALUE
    ...
    if (!cacheFileFinal.exists()) {
      ...
      if (cacheFileTemp.exists()) {
        ...
        nextDownloadOffset =
            downloadedBytes = downloadedBytes / currentDownloadChunkSize * currentDownloadChunkSize;
      }
      if (fileNameIv != null) {
      ...
      try {
        ...
        if (key != null) {
          Utilities.aesIgeEncryption(
              ...
              true,
              0,
              requestInfo.response.bytes.limit());
        }
        if (fileOutputStream != null) {
        ...
        if (totalBytesCount > 0 && state == stateDownloading) {
          delegate.didChangedLoadProgress(
              FileLoadOperation.this,
              Math.min(1.0f, (float) downloadedBytes / (float) totalBytesCount));
        }
        ...
        if (currentBytesSize != currentDownloadChunkSize) {
          ...
          if (totalBytesCount != downloadedBytes && downloadedBytes % currentDownloadChunkSize == 0
              || totalBytesCount > 0 && totalBytesCount > downloadedBytes) {
            startDownloadRequest();
          } else {
            onFinishLoadingFile();
          }
  ...
  private void startDownloadRequest() {
    if (state != stateDownloading
        || totalBytesCount > 0 && nextDownloadOffset >= totalBytesCount
        || requestInfos.size() + delayedRequestInfos.size() >= maxDownloadRequests) {
      return;
    }
    int count = 1;
    ...
    for (int a = 0; a < count; a++) {
      ...
      boolean isLast =
          totalBytesCount <= 0
              || a == count - 1
              || totalBytesCount > 0
                  && nextDownloadOffset + currentDownloadChunkSize >= totalBytesCount;
      ...
      requestInfo.requestToken =
          ConnectionsManager.getInstance()
              .performRpc(
                  ...
                  null,
                  true,
                  RPCRequest.RPCRequestClassDownloadMedia
                      | (isForceRequest ? RPCRequest.RPCRequestClassForceDownload : 0),
                  datacenter_id,
                  isLast);
    }
  }
