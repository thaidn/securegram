...
public class ID3v2FrameHeader {
  ...
  public ID3v2FrameHeader(ID3v2TagBody input) throws IOException, ID3v2Exception {
    ...
    if (input.getTagHeader().getVersion() > 2) { // $xx xx
      ...
      if (input.getTagHeader().getVersion()
          == 3) { // %(compression)(encryption)(groupingIdentity)00000
  ...
  public boolean isValid() {
    for (int i = 0; i < frameId.length(); i++) {
      if ((frameId.charAt(i) < 'A' || frameId.charAt(i) > 'Z')
          && (frameId.charAt(i) < '0' || frameId.charAt(i) > '9')) {
        return false;
      }
    }
    return bodySize > 0;
