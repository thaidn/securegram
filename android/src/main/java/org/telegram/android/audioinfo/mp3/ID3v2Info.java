...
public class ID3v2Info extends AudioInfo {
            ...
            try {
              ...
              if (LOGGER.isLoggable(debugLevel)) {
                LOGGER.log(
                    debugLevel,
                    String.format(
                        "ID3 exception occured in frame %s: %s",
                        frameHeader.getFrameId(),
                        e.getMessage()));
              }
            } finally {
    ...
    switch (frame.getFrameHeader().getFrameId()) {
      ...
      case "APIC": // cover: prefer TYPE_COVER_FRONT, then TYPE_OTHER, then anything else
        if (cover == null || coverPictureType != AttachedPicture.TYPE_COVER_FRONT) {
          ...
          if (cover == null
              || picture.type == AttachedPicture.TYPE_COVER_FRONT
              || picture.type == AttachedPicture.TYPE_OTHER) {
            try {
      ...
      case "COMM":
        ...
        if (comment == null
            ...
            || "".equals(comm.description)) { // prefer "default" comment (without description)
          comment = comm.text;
        }
        break;
      case "TAL":
  ...
  CommentOrUnsynchronizedLyrics parseCommentOrUnsynchronizedLyricsFrame(ID3v2FrameBody data)
  ...
  AttachedPicture parseAttachedPictureFrame(ID3v2FrameBody data)
      throws IOException, ID3v2Exception {
    ID3v2Encoding encoding = data.readEncoding();
    String imageType;
