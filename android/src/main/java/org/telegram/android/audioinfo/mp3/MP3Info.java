...
public class MP3Info extends AudioInfo {
  ...
  public MP3Info(InputStream input, long fileLength)
      throws IOException, ID3v2Exception, MP3Exception {
    this(input, fileLength, Level.FINEST);
  }

  public MP3Info(InputStream input, final long fileLength, Level debugLevel)
      throws IOException, ID3v2Exception, MP3Exception {
    brand = "MP3";
    version = "0";
    ...
    if (duration <= 0
        || duration
            >= 3600000L) { // don't trust strange durations (e.g. old lame versions always write TLEN 97391548)
      try {
        duration =
            calculateDuration(
                data,
                fileLength,
                new StopReadCondition() {
                  final long stopPosition = fileLength - 128;

                  @Override
                  public boolean stopRead(MP3Input data) throws IOException {
                    return (data.getPosition() == stopPosition)
                        && ID3v1Info.isID3v1StartPosition(data);
                  }
                });
      ...
      if (b0 == 0xFF && (b1 & 0xE0) == 0xE0) { // first 11 bits should be 1
        ...
        if (header != null) { // we have a candidate
          ...
          if (!frame.isChecksumError()) {
            ...
            if (nextB0 == 0xFF
                && (nextB1 & 0xFE)
                    == (b1 & 0xFE)) { // quick check: nextB1 must match b1's version & layer
  ...
  MP3Frame readNextFrame(MP3Input data, StopReadCondition stopCondition, MP3Frame previousFrame)
      throws IOException {
  ...
  long calculateDuration(MP3Input data, long totalLength, StopReadCondition stopCondition)
