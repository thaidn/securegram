...
public class StickersQuery {
  ...
  public static void checkStickers() {
    if (!loadingStickers
        && (!stickersLoaded || loadDate < (System.currentTimeMillis() / 1000 - 60 * 60))) {
                ...
                public void run() {
                  ...
                  try {
                    SQLiteCursor cursor =
                        MessagesStorage.getInstance()
                            .getDatabase()
                            .queryFinalized("SELECT data, date, hash FROM stickers_v2 WHERE 1");
                    if (cursor.next()) {
                      ByteBufferDesc data =
                          MessagesStorage.getInstance()
                              .getBuffersStorage()
                      ...
                      new Runnable() {
                        ...
                        public void run() {
                          if (response instanceof TLRPC.TL_messages_allStickers) {
                            final HashMap<Long, TLRPC.TL_messages_stickerSet> newStickerSets =
                                new HashMap<>();
                            final ArrayList<TLRPC.TL_messages_stickerSet> newStickerArray =
                                new ArrayList<>();
                            final TLRPC.TL_messages_allStickers res =
                                (TLRPC.TL_messages_allStickers) response;

                            for (int a = 0; a < res.sets.size(); a++) {
                              final TLRPC.StickerSet stickerSet = res.sets.get(a);

                              TLRPC.TL_messages_stickerSet oldSet =
                                  stickerSetsById.get(stickerSet.id);
                              ...
                              TLRPC.TL_messages_getStickerSet req =
                                        ...
                                        public void run(
                                          ...
                                          AndroidUtilities.runOnUIThread(
                                              new Runnable() {
                                                @Override
                                                public void run() {
                                                  TLRPC.TL_messages_stickerSet res1 =
                                                      (TLRPC.TL_messages_stickerSet) response;
  ...
  private static void putStickersToCache(
      final ArrayList<TLRPC.TL_messages_stickerSet> stickers, final int date, final String hash) {
    MessagesStorage.getInstance()
        .getStorageQueue()
        .postRunnable(
            new Runnable() {
              ...
              public void run() {
                try {
                  SQLitePreparedStatement state =
                      MessagesStorage.getInstance()
                          .getDatabase()
                          .executeFast("REPLACE INTO stickers_v2 VALUES(?, ?, ?, ?)");
                  state.requery();
                  int size = 4;
                  ...
                  ByteBufferDesc data =
                      MessagesStorage.getInstance().getBuffersStorage().getFreeBuffer(size);
  ...
  private static void processLoadedStickers(
      final ArrayList<TLRPC.TL_messages_stickerSet> res,
      final boolean cache,
      final int date,
      final String hash) {
    AndroidUtilities.runOnUIThread(
    ...
    Utilities.stageQueue.postRunnable(
        new Runnable() {
          @Override
          public void run() {
            if (cache && (res == null || date < (int) (System.currentTimeMillis() / 1000 - 60 * 60))
                || !cache && res == null && hash == null) {
              AndroidUtilities.runOnUIThread(
                  new Runnable() {
                    @Override
                    public void run() {
            ...
            if (res != null) {
              ...
              final HashMap<Long, TLRPC.TL_messages_stickerSet> stickerSetsByIdNew =
                  new HashMap<>();
                  ...
                  new Runnable() {
                    ...
                    public void run() {
                      ...
                      NotificationCenter.getInstance()
                          .postNotificationName(NotificationCenter.stickersDidLoaded);
                    }
                  });
            }
          }
  ...
  public static void loadStickers(
      final BaseFragment fragment, final TLRPC.InputStickerSet stickerSet) {
                        ...
                        new Runnable() {
                          ...
                          public void run() {
                            ...
                            if (fragment != null
                              ...
                              if (error == null) {
                                final TLRPC.TL_messages_stickerSet res =
                                    (TLRPC.TL_messages_stickerSet) response;

                                StickersAlert alert =
                                    new StickersAlert(fragment.getParentActivity(), res);
                                if (res.set == null
                                    || !StickersQuery.isStickerPackInstalled(res.set.id)) {
                                  alert.setButton(
                                      AlertDialog.BUTTON_POSITIVE,
                                      LocaleController.getString(
                                          "AddStickers", R.string.AddStickers),
                                  ...
                                  alert.setButton(
                                      ...
                                      new DialogInterface.OnClickListener() {
                                        ...
                                        public void onClick(DialogInterface dialog, int which) {
                                          removeStickersSet(
                                              fragment.getParentActivity(), res.set, 0);
                                        }
                                      });
                                }
                                alert.setButton(
                                ...
                                Toast.makeText(
                                        fragment.getParentActivity(),
                                        LocaleController.getString(
                                            "AddStickersNotFound", R.string.AddStickersNotFound),
                                        Toast.LENGTH_SHORT)
                                    .show();
                              }
                            }
  ...
  public static void removeStickersSet(
      final Context context, TLRPC.StickerSet stickerSet, int hide) {
                          ...
                          try {
                            if (error == null) {
                              Toast.makeText(
                                      context,
                                      LocaleController.getString(
                                          "StickersRemoved", R.string.StickersRemoved),
                                      Toast.LENGTH_SHORT)
                                  .show();
                            } else {
                              Toast.makeText(
                                      context,
                                      LocaleController.getString(
                                          "ErrorOccurred", R.string.ErrorOccurred),
                                      Toast.LENGTH_SHORT)
                                  .show();
                            }
                          } catch (Exception e) {
