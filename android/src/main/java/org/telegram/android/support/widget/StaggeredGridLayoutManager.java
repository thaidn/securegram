...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager {
  ...
  private boolean checkForGaps() {
    ...
    final LazySpanLookup.FullSpanItem invalidFsi =
        mLazySpanLookup.getFirstFullSpanItemInRange(minPos, maxPos + 1, invalidGapDir, true);
    if (invalidFsi == null) {
      mLaidOutInvalidFullSpan = false;
      mLazySpanLookup.forceInvalidateAfter(maxPos + 1);
      return false;
    }
    final LazySpanLookup.FullSpanItem validFsi =
        mLazySpanLookup.getFirstFullSpanItemInRange(
            minPos, invalidFsi.mPosition, invalidGapDir * -1, true);
    if (validFsi == null) {
  ...
  public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    if (getChildCount() > 0
        && (mPendingSavedState == null || mPendingSavedState.mSpanOffsetsSize < 1)) {
    ...
    if (!state.isPreLayout()) {
      final boolean needToCheckForGaps =
          mGapStrategy != GAP_HANDLING_NONE
              && getChildCount() > 0
  ...
  boolean updateAnchorFromPendingData(RecyclerView.State state, AnchorInfo anchorInfo) {
    ...
    if (mPendingSavedState == null
        || mPendingSavedState.mAnchorPosition == NO_POSITION
      ...
      if (child != null) {
        ...
        // child
        anchorInfo.mPosition =
            mShouldReverseLayout ? getLastChildPosition() : getFirstChildPosition();

        if (mPendingScrollPositionOffset != INVALID_OFFSET) {
          if (anchorInfo.mLayoutFromEnd) {
            final int target =
                mPrimaryOrientation.getEndAfterPadding() - mPendingScrollPositionOffset;
            anchorInfo.mOffset = target - mPrimaryOrientation.getDecoratedEnd(child);
          } else {
            final int target =
                mPrimaryOrientation.getStartAfterPadding() + mPendingScrollPositionOffset;
        ...
        final int startGap =
            mPrimaryOrientation.getDecoratedStart(child)
                - mPrimaryOrientation.getStartAfterPadding();
        if (startGap < 0) {
        ...
        final int endGap =
            mPrimaryOrientation.getEndAfterPadding() - mPrimaryOrientation.getDecoratedEnd(child);
        if (endGap < 0) {
  ...
  public int[] findFirstVisibleItemPositions(int[] into) {
    if (into == null) {
      ...
      throw new IllegalArgumentException(
          "Provided int[]'s size must be more than or equal"
    ...
    if (lp.mFullSpan) {
      if (mOrientation == VERTICAL) {
        measureChildWithDecorationsAndMargin(
            child, mFullSizeSpec, getSpecForDimension(lp.height, mHeightSpec));
      } else {
        measureChildWithDecorationsAndMargin(
            child, getSpecForDimension(lp.width, mWidthSpec), mFullSizeSpec);
      }
    } else {
      if (mOrientation == VERTICAL) {
        measureChildWithDecorationsAndMargin(
            child, mWidthSpec, getSpecForDimension(lp.height, mHeightSpec));
      } else {
        measureChildWithDecorationsAndMargin(
            child, getSpecForDimension(lp.width, mWidthSpec), mHeightSpec);
      }
    }
  }
  ...
  private void measureChildWithDecorationsAndMargin(View child, int widthSpec, int heightSpec) {
    ...
    widthSpec =
        updateSpecWithExtra(
            widthSpec, lp.leftMargin + mTmpRect.left, lp.rightMargin + mTmpRect.right);
    heightSpec =
        updateSpecWithExtra(
            heightSpec, lp.topMargin + mTmpRect.top, lp.bottomMargin + mTmpRect.bottom);
    ...
    if (mOrientation == HORIZONTAL) {
      info.setCollectionItemInfo(
          AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(
              sglp.getSpanIndex(), sglp.mFullSpan ? mSpanCount : 1, -1, -1, sglp.mFullSpan, false));
    } else { // VERTICAL
      info.setCollectionItemInfo(
          AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(
              -1, -1, sglp.getSpanIndex(), sglp.mFullSpan ? mSpanCount : 1, sglp.mFullSpan, false));
    }
  }
  ...
  private void fixEndGap(
      RecyclerView.Recycler recycler, RecyclerView.State state, boolean canOffsetChildren) {
  ...
  private void setLayoutStateDirection(int direction) {
    ...
    mLayoutState.mItemDirection =
        (mShouldReverseLayout == (direction == LAYOUT_START))
            ? ITEM_DIRECTION_TAIL
            : ITEM_DIRECTION_HEAD;
  }
    ...
    while (layoutState.hasMore(state) && !mRemainingSpans.isEmpty()) {
      ...
      if (lp.mFullSpan && layoutState.mItemDirection == ITEM_DIRECTION_HEAD) {
        if (assignSpan) {
          ...
          if (hasInvalidGap) {
            final LazySpanLookup.FullSpanItem fullSpanItem =
                mLazySpanLookup.getFullSpanItem(position);
    ...
    void calculateCachedStart() {
      ...
      if (lp.mFullSpan) {
        LazySpanLookup.FullSpanItem fsi =
            mLazySpanLookup.getFullSpanItem(lp.getViewLayoutPosition());
  ...
  static class LazySpanLookup {
     ...
     */
    public FullSpanItem getFirstFullSpanItemInRange(
        int minPos, int maxPos, int gapDir, boolean hasUnwantedGapAfter) {
      if (mFullSpanItems == null) {
        return null;
      }
