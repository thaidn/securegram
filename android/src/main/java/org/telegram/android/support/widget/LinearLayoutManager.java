...
public class LinearLayoutManager extends RecyclerView.LayoutManager
  ...
  public Parcelable onSaveInstanceState() {
    ...
    if (getChildCount() > 0) {
      ...
      if (didLayoutFromEnd) {
        ...
        state.mAnchorPosition = getPosition(refChild);
        state.mAnchorOffset =
            mOrientationHelper.getDecoratedStart(refChild)
   ...
   */
  private void layoutForPredictiveAnimations(
      RecyclerView.Recycler recycler, RecyclerView.State state, int startOffset, int endOffset) {
    ...
    for (int i = 0; i < scrapSize; i++) {
      ...
      final int direction =
          position < firstChildPos != mShouldReverseLayout
    ...
    if (DEBUG) {
      Log.d(
          TAG,
          "for unused scrap, decided to add "
              + scrapExtraStart
              + " towards start and "
              + scrapExtraEnd
              + " towards end");
  ...
  private boolean updateAnchorFromPendingData(RecyclerView.State state, AnchorInfo anchorInfo) {
    ...
    if (mPendingSavedState != null && mPendingSavedState.hasValidAnchor()) {
      ...
      if (anchorInfo.mLayoutFromEnd) {
        anchorInfo.mCoordinate =
            mOrientationHelper.getEndAfterPadding() - mPendingSavedState.mAnchorOffset;
      } else {
        anchorInfo.mCoordinate =
            mOrientationHelper.getStartAfterPadding() + mPendingSavedState.mAnchorOffset;
      }
      return true;
    }
    ...
    if (mPendingScrollPositionOffset == INVALID_OFFSET) {
      ...
      if (child != null) {
        ...
        final int startGap =
            mOrientationHelper.getDecoratedStart(child) - mOrientationHelper.getStartAfterPadding();
        if (startGap < 0) {
        ...
        final int endGap =
            mOrientationHelper.getEndAfterPadding() - mOrientationHelper.getDecoratedEnd(child);
        if (endGap < 0) {
    ...
    if (mShouldReverseLayout) {
      anchorInfo.mCoordinate =
          mOrientationHelper.getEndAfterPadding() - mPendingScrollPositionOffset;
    } else {
      anchorInfo.mCoordinate =
          mOrientationHelper.getStartAfterPadding() + mPendingScrollPositionOffset;
    }
    return true;
  }
  ...
  private void updateLayoutStateToFillEnd(int itemPosition, int offset) {
    ...
    mLayoutState.mItemDirection =
        mShouldReverseLayout ? LayoutState.ITEM_DIRECTION_HEAD : LayoutState.ITEM_DIRECTION_TAIL;
  ...
  private void updateLayoutStateToFillStart(int itemPosition, int offset) {
    ...
    mLayoutState.mCurrentPosition = itemPosition;
    mLayoutState.mItemDirection =
        mShouldReverseLayout ? LayoutState.ITEM_DIRECTION_TAIL : LayoutState.ITEM_DIRECTION_HEAD;
  ...
  private void updateLayoutState(
    ...
    if (layoutDirection == LayoutState.LAYOUT_END) {
      ...
      fastScrollSpace =
          mOrientationHelper.getDecoratedEnd(child) - mOrientationHelper.getEndAfterPadding();
  ...
  private View findFirstVisibleChildClosestToStart(
    ...
    if (mShouldReverseLayout) {
      return findOneVisibleChild(
          getChildCount() - 1, -1, completelyVisible, acceptPartiallyVisible);
    } else {
  ...
  private void logChildren() {
    ...
    for (int i = 0; i < getChildCount(); i++) {
      ...
      Log.d(
          TAG,
          "item " + getPosition(child) + ", coord:" + mOrientationHelper.getDecoratedStart(child));
    }
    Log.d(TAG, "==============");
  }
      ...
      for (int i = 1; i < getChildCount(); i++) {
        ...
        if (pos < lastPos) {
          ...
          throw new RuntimeException(
              "detected invalid position. loc invalid? " + (screenLoc < lastScreenLoc));
        }
  ...
  public void prepareForDrop(View view, View target, int x, int y) {
    ...
    final int dropDirection =
        myPos < targetPos ? LayoutState.ITEM_DIRECTION_TAIL : LayoutState.ITEM_DIRECTION_HEAD;
    if (mShouldReverseLayout) {
      if (dropDirection == LayoutState.ITEM_DIRECTION_TAIL) {
        ...
        scrollToPositionWithOffset(
            targetPos,
            mOrientationHelper.getEndAfterPadding() - mOrientationHelper.getDecoratedEnd(target));
      }
    } else {
  ...
  static class LayoutState {
    ...
    public void assignPositionFromScrapList(RecyclerView.ViewHolder ignore) {
      ...
      mCurrentPosition = closest == null ? RecyclerView.NO_POSITION : closest.getLayoutPosition();
    }
  ...
  class AnchorInfo {
    ...
    public void assignFromViewAndKeepVisibleRect(View child) {
      ...
      if (mLayoutFromEnd) {
        ...
        if (startMargin > 0) { // we have room to fix end as well
          ...
          final int previousEndMargin =
              previousLayoutEnd - mOrientationHelper.getDecoratedEnd(child);
          final int endReference =
              mOrientationHelper.getEndAfterPadding() - Math.min(0, previousEndMargin);
    ...
    public void assignFromView(View child) {
      if (mLayoutFromEnd) {
        mCoordinate =
            mOrientationHelper.getDecoratedEnd(child) + mOrientationHelper.getTotalSpaceChange();
      } else {
