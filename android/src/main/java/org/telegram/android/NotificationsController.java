...
public class NotificationsController {
  ...
  public NotificationsController() {
    ...
    try {
      alarmManager =
          (AlarmManager)
              ApplicationLoader.applicationContext.getSystemService(Context.ALARM_SERVICE);
    } catch (Exception e) {
      FileLog.e("tmessages", e);
    }
          ...
          if (messageObject.messageOwner instanceof TLRPC.TL_messageService) {
            if (messageObject.messageOwner.action instanceof TLRPC.TL_messageActionUserJoined) {
              ...
              msg =
                ...
                instanceof TLRPC.TL_messageActionLoginUnknownLocation) {
              String date =
                  LocaleController.formatString(
                      "formatDateAtTime",
                      R.string.formatDateAtTime,
                      LocaleController.formatterYear.format(
                          ((long) messageObject.messageOwner.date) * 1000),
              ...
              msg =
                  LocaleController.formatString(
                      ...
                      UserConfig.getCurrentUser().first_name,
                      date,
              ...
              if (messageObject.isSticker()) {
                msg =
                    LocaleController.formatString(
                        ...
                        R.string.NotificationMessageSticker,
                        UserObject.getUserName(user));
              } else {
                msg =
                    LocaleController.formatString(
          ...
          if (messageObject.messageOwner instanceof TLRPC.TL_messageService) {
            if (messageObject.messageOwner.action instanceof TLRPC.TL_messageActionChatAddUser) {
              if (messageObject.messageOwner.action.user_id == UserConfig.getClientUserId()) {
                ...
                if (user.id == u2.id) {
                  msg =
                      LocaleController.formatString(
                          ...
                          R.string.NotificationGroupAddSelf,
                          UserObject.getUserName(user),
                          chat.title);
                } else {
                  msg =
                ...
                instanceof TLRPC.TL_messageActionChatJoinedByLink) {
              msg =
                  LocaleController.formatString(
                      "NotificationInvitedToGroupByLink",
                      R.string.NotificationInvitedToGroupByLink,
                      UserObject.getUserName(user),
                      chat.title);
            } else if (messageObject.messageOwner.action
              ...
              msg =
                  LocaleController.formatString(
                    ...
                    instanceof TLRPC.TL_messageActionChatEditPhoto
                || messageObject.messageOwner.action
                    instanceof TLRPC.TL_messageActionChatDeletePhoto) {
              msg =
                  LocaleController.formatString(
              ...
              if (messageObject.messageOwner.action.user_id == UserConfig.getClientUserId()) {
                ...
                instanceof TLRPC.TL_messageActionChatCreate) {
              ...
              if (messageObject.isSticker()) {
                msg =
                    LocaleController.formatString(
                        ...
                        R.string.NotificationMessageGroupSticker,
                        UserObject.getUserName(user),
                        chat.title);
              } else {
                msg =
    ...
    try {
      PendingIntent pintent =
          PendingIntent.getService(
              ...
              0,
              new Intent(ApplicationLoader.applicationContext, NotificationRepeat.class),
              0);
      SharedPreferences preferences =
          ApplicationLoader.applicationContext.getSharedPreferences(
  ...
  private void showOrUpdateNotification(boolean notifyAboutLast) {
    ...
    try {
      ...
      if (!notifyDisabled) {
        ...
        if (needVibrate == 2
                && (vibrateOverride == 1 || vibrateOverride == 3 || vibrateOverride == 5)
      ...
      PendingIntent contentIntent =
          PendingIntent.getActivity(
              ApplicationLoader.applicationContext, 0, intent, PendingIntent.FLAG_ONE_SHOT);
        ...
        if (choosenSoundPath != null && !choosenSoundPath.equals("NoSound")) {
          if (choosenSoundPath.equals(defaultPath)) {
            /*MediaPlayer mediaPlayer = new MediaPlayer();
             ...
             mediaPlayer.start();*/
            mBuilder.setSound(
                Settings.System.DEFAULT_NOTIFICATION_URI, AudioManager.STREAM_NOTIFICATION);
          } else {
  ...
  public void showExtraNotifications(
      NotificationCompat.Builder notificationBuilder, boolean notifyAboutLast) {
    ...
    for (int b = 0; b < sortedDialogs.size(); b++) {
      ...
      PendingIntent msgReplyPendingIntent =
          PendingIntent.getBroadcast(
              ...
              msgReplyIntent,
              PendingIntent.FLAG_UPDATE_CURRENT);
      RemoteInput remoteInputAuto =
          new RemoteInput.Builder(NotificationsController.EXTRA_VOICE_REPLY)
      ...
      Intent replyIntent =
          new Intent(ApplicationLoader.applicationContext, WearReplyReceiver.class);
      ...
      NotificationCompat.Action action =
          new NotificationCompat.Action.Builder(
                  R.drawable.ic_reply_icon, replyToString, replyPendingIntent)
              .addRemoteInput(remoteInputWear).build();
  ...
  public void processReadMessages(
      HashMap<Integer, Integer> inbox, long dialog_id, int max_date, int max_id, boolean isPopup) {
    int oldCount = popupMessages.size();
    if (inbox != null) {
      for (HashMap.Entry<Integer, Integer> entry : inbox.entrySet()) {
        for (int a = 0; a < pushMessages.size(); a++) {
          MessageObject messageObject = pushMessages.get(a);
          if (messageObject.getDialogId() == entry.getKey()
              && messageObject.getId() <= entry.getValue()) {
  ...
  public void processNewMessages(ArrayList<MessageObject> messageObjects, boolean isLast) {
    ...
    for (int a = 0; a < messageObjects.size(); a++) {
      ...
      if (value == null) {
        ...
        value =
            !(notifyOverride == 2
                || (!preferences.getBoolean("EnableAll", true)
                        || isChat && !preferences.getBoolean("EnableGroup", true))
                    && notifyOverride == 0);
    ...
    if (!popupMessages.isEmpty()
        && oldCount != popupMessages.size()
        && !AndroidUtilities.needShowPasscode(false)
        && !UserConfig.isWaitingForPasscodeEnter) {
      if (ApplicationLoader.mainInterfacePaused || !ApplicationLoader.isScreenOn) {
        MessageObject messageObject = messageObjects.get(0);
        if (popup == 3
            || popup == 1 && ApplicationLoader.isScreenOn
            || popup == 2 && !ApplicationLoader.isScreenOn) {
          Intent popupIntent =
              new Intent(ApplicationLoader.applicationContext, PopupNotificationActivity.class);
  ...
  private boolean isPersonalMessage(MessageObject messageObject) {
    return messageObject.messageOwner.to_id != null
        && messageObject.messageOwner.to_id.chat_id == 0
